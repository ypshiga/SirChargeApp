...
if item_select:
    ...
    if len(df_temp)>1:
        ...
        if len(cpt_pick)>1:
            ...
            col1_a.write('CPT codes: ' + ", ".join(cpt_text))
        else:
...
# ## add subset by categories (by CPT code)
# array_dict = {} 
# appended_df_2 = appended_df.iloc[(appended_df.groupby('Item Name')["Average Charge"].transform('mean')).argsort()]
# appended_df_2 = appended_df_2.drop(appended_df_2.index[appended_df_2['lat']== 0])
# for i,item in enumerate(appended_df_2['Item Name'].unique()):
    # array_dict[item] = appended_df["Average Charge"][appended_df['Item Name']==item]

# colors = n_colors('rgb(5, 200, 200)', 'rgb(200, 10, 10)', appended_df_2['Item Name'].nunique(), colortype='rgb')

# fig = go.Figure()
# for data_line, color in zip((appended_df_2['Item Name'].unique()), colors):
    # c_val = appended_df_2['2020 CPT Code'][appended_df_2['Item Name'] == data_line].unique()
    # fig.add_trace(go.Violin(x=array_dict[data_line], line_color=color,name = "".join(str(int(c_val[0])))))

# fig.update_traces(orientation='h', side='positive', width=2, points=False,showlegend=False)
# fig.update_layout(violingap=1,xaxis_showgrid=False, xaxis_zeroline=False,height=1000)
# #fig.update_xaxes(range=[0,3])
# ##st.plotly_chart(fig,use_container_width=True)
# st.plotly_chart(fig)

#st.write(appended_df.head())
